...
export class ExamensNationauxSubmissionsService {
    ...
    private withMissingFlags(s: ExamenNationalSubmission) {
        ...
        return {
            ...
            missing_matiere: s.matiere_filiere_examen_id == null,
    ...
    private async loadOrThrow(pays: string, id: number): Promise<ExamenNationalSubmission> {
        const s = await this.subRepo.findOne({
            where: { id, pays },
            relations: ['type_examen', 'serie', 'matiere_filiere_examen', 'soumis_par'],
        });
    ...
    async createSubmission(pays: string, userId: number, dto: CreateExamenNationalSubmissionDto) {
        ...
        const proposedMatiere = dto.proposed_matiere_filiere?.trim() || null;

        const hasType = dto.type_examen_id != null || !!proposedType;
        const hasMatiere = dto.matiere_filiere_examen_id != null || !!proposedMatiere;
        if (!hasType || !hasMatiere) {
            throw new BadRequestException('Le type et la matière/filière sont requis (id existant ou nom proposé).');
        }
        ...
        if (dto.matiere_filiere_examen_id != null && !(await this.mfeRepo.findOne({ where: { id: dto.matiere_filiere_examen_id } }))) {
            throw new NotFoundException(`Matière/filière ${dto.matiere_filiere_examen_id} introuvable`);
        }
        ...
        const submission = this.subRepo.create({
            ...
            matiere_filiere_examen_id: dto.matiere_filiere_examen_id ?? null,
            proposed_matiere_filiere: dto.matiere_filiere_examen_id != null ? null : proposedMatiere,
    ...
    async findAll(pays: string, status: string | undefined, paginationDto: PaginationDto): Promise<PaginationResponse<any>> {
        ...
        const qb = this.subRepo.createQueryBuilder('s')
            ...
            .leftJoinAndSelect('s.matiere_filiere_examen', 'matiere_filiere_examen')
    ...
    async resolveSubmission(pays: string, id: number, resolve: ResoudreExamenNationalSubmissionDto) {
        ...
        if (resolve.matiere_filiere_examen_id != null) {
            if (!(await this.mfeRepo.findOne({ where: { id: resolve.matiere_filiere_examen_id } }))) throw new NotFoundException(`Matière/filière ${resolve.matiere_filiere_examen_id} introuvable`);
            patch.matiere_filiere_examen_id = resolve.matiere_filiere_examen_id; patch.proposed_matiere_filiere = null;
        } else if (resolve.proposed_matiere_filiere !== undefined) {
            patch.proposed_matiere_filiere = resolve.proposed_matiere_filiere?.trim() || null;
            if (patch.proposed_matiere_filiere) patch.matiere_filiere_examen_id = null;
        }
    ...
    async approve(pays: string, id: number, resolve?: { type_examen_id?: number; serie_id?: number; matiere_filiere_examen_id?: number }) {
        const submission = await this.loadOrThrow(pays, id);
        ...
        if (resolve?.matiere_filiere_examen_id != null) { submission.matiere_filiere_examen_id = resolve.matiere_filiere_examen_id; submission.proposed_matiere_filiere = null; }

        if (submission.type_examen_id == null || submission.matiere_filiere_examen_id == null) {
            throw new BadRequestException('Le type et la matière/filière doivent être résolus avant approbation.');
        }
        ...
        const matiere = await this.mfeRepo.findOne({ where: { id: submission.matiere_filiere_examen_id } });
        ...
        if (!matiere) throw new NotFoundException(`Matière/filière ${submission.matiere_filiere_examen_id} introuvable`);

        const titre = [type.nom, serie?.nom, matiere.nom, String(submission.annee)].filter(Boolean).join(' - ');
        const examen = this.examRepo.create({
            ...
            matiere_filiere_examen_id: submission.matiere_filiere_examen_id,
    ...
    async decline(pays: string, id: number, reason?: string) {
        ...
        const title = submission.type_examen?.nom
            ? [submission.type_examen?.nom, submission.serie?.nom, submission.matiere_filiere_examen?.nom].filter(Boolean).join(' - ')
            : (submission.proposed_type || submission.proposed_matiere_filiere || 'Examen national soumis');
