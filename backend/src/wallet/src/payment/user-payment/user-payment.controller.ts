...
//   @Post('accounts')
//   @ApiOperation({ summary: 'Créer ou mettre à jour le compte Mobile Money par défaut' })
//   upsertAccount(@Request() req, @Body() dto: UpsertPaymentAccountDto) {
//     return this.upsertPaymentAccount.execute({ userId: req.user.utilisateurId, changedBy: req.user.utilisateurId, ...dto });
//   }

//   @Get('accounts/me')
//   @ApiOperation({ summary: 'Lister ses comptes de paiement' })
...
//   adminWithdrawals(@Query('status') status?: WithdrawalStatus, @Query('page') page?: string, @Query('limit') limit?: string) {
//     return this.listAdminWithdrawals.execute(status, Number(page ?? 1), Number(limit ?? 20));
//   }
...
//   reject(@Request() req, @Param('id') id: string, @Body() dto: RejectWithdrawalDto) {
//     return this.rejectWithdrawal.execute(id, req.user.utilisateurId, dto.reason);
//   }

//   @UseGuards(RoleGuard)
//   @Roles(RoleType.ADMIN)
//   @Post('admin/withdrawals/:id/confirm-payment')
//   confirmPayment(@Request() req, @Param('id') id: string, @Body() dto: ConfirmManualPaymentDto) {
...
//   updatePaymentConfiguration(@Request() req, @Body() dto: UpdatePaymentConfigurationDto) {
